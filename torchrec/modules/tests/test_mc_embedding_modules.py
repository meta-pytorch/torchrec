...
class MCHManagedCollisionEmbeddingBagCollectionTest(unittest.TestCase):
    ...
    def test_mc_ebc_ec_forward_is_nro_feature_parameter(self) -> None:
        """
        Test that ManagedCollisionEmbeddingBagCollection and ManagedCollisionEmbeddingCollection
        correctly accept and pass the is_nro_feature parameter to the underlying
        ...
        # Test ManagedCollisionEmbeddingBagCollection with is_nro_feature=True (default)
        out_ebc_true, remapped_kjt_ebc_true = mc_ebc.forward(kjt, is_nro_feature=True)
        self.assertIsNotNone(out_ebc_true)
        self.assertIsNotNone(remapped_kjt_ebc_true)

        # Test ManagedCollisionEmbeddingBagCollection with is_nro_feature=False
        out_ebc_false, remapped_kjt_ebc_false = mc_ebc.forward(
            kjt, is_nro_feature=False
        )
        self.assertIsNotNone(out_ebc_false)
        self.assertIsNotNone(remapped_kjt_ebc_false)

        # Test ManagedCollisionEmbeddingCollection with is_nro_feature=True (default)
        out_ec_true, remapped_kjt_ec_true = mc_ec.forward(kjt, is_nro_feature=True)
        self.assertIsNotNone(out_ec_true)
        self.assertIsNotNone(remapped_kjt_ec_true)

        # Test ManagedCollisionEmbeddingCollection with is_nro_feature=False
        out_ec_false, remapped_kjt_ec_false = mc_ec.forward(kjt, is_nro_feature=False)
