...
def _shard_embeddings_in_hsd(
    ...
    Two different mechanisms, one per half:

    * **sparse** -- DMP shards the tables over ``stage_pg``, so the lookup
      all-to-all stays local to the HSD with no global cross-HSD exchange, and
      the embedding optimizer is fused into the TBE backward.
    * **dense** -- ``replicate`` over the same group, i.e. FSDP2 in its ZeRO-0
      shape, reduced once per pass by the schedule.

    Dense used to go through DMP's own ``DefaultDataParallelWrapper``, which
    builds raw DDP. ``MaglevRail`` cannot reduce that: its split backward writes
    ``.grad`` without running ``AccumulateGrad``, and DDP's reducer is driven
    only by the hooks on that node, with no entry point to drive it by hand.
    FSDP2 does expose one (``FSDPParamGroup.post_backward``), so both schedules
    use it here -- ``Maglev1F1B`` through ``_configure_fsdp_backward``, and
    ``MaglevRail`` through its end-of-pass reduction.

    ``replicate`` goes on each layer's **dense half**, not the layer: applied to
    a whole layer it would try to shard the table DMP has already sharded, and
    ...
    dmp = DistributedModelParallel(
        ...
        plan=plan,
        sharders=sharders,
        # No DDP: the dense halves are replicated with FSDP2 below instead.
    ...
    for layer in dmp.modules():
        if isinstance(layer, MaglevLayer) and layer.dense is not None:
            ...
            if dp_mode == "zero3":
                # Explicit, so the two schedules are comparable: MaglevRail
                # forces this off for itself, but Maglev1F1B does not, and at
                # the default the baseline would pay one all-gather per
                # microbatch (measured 96 vs 12 at m=8) against Rail's one per
                # pass. That is a property of the flag, not of the schedule.
