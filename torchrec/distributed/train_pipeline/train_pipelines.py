...
class TrainEvalHybridPipelineBase(TrainPipelineSparseDist[In, Out]):
    ...
    def enqueue_batch(self, dataloader_iter: Iterator[In], is_training: bool) -> bool:
        """
        Enqueue a new batch from the dataloader into the pipeline.

        This method extends the parent class's `enqueue_batch` to support hybrid
        train/eval mode by setting the `is_training` flag on the batch context.

        Args:
            dataloader_iter: Iterator yielding input batches from the dataloader.
            is_training: Whether the batch should be processed in training mode.
                If False, the context's `is_training` flag is set to False,
                which will skip backward pass and optimizer step for this batch.

        Returns:
            bool: True if a batch was successfully enqueued, False if the
                dataloader iterator is exhausted.
        """
        rt = super().enqueue_batch(dataloader_iter)
        # Mark the newly enqueued batch's context as eval-only if not training
        if not is_training:
            self.contexts[-1].is_training = False
        return rt
    ...
    def progress(self, dataloader_iter: Iterator[In], is_training: bool = True) -> Out:
        ...
        Note:
            ...
            2. `self.context[0].is_training` - the per-batch training flag set during enqueue
        ...
        # 2. Current batch is marked for training (self.context[0].is_training)
        if self._model.training and self.context[0].is_training:
            # Backward pass
            self._state = PipelineState.CALL_BWD
            self._backward(losses)
