    ...
    def _get_virtual_table_feature_num_buckets(self) -> Tuple[List[int], bool]:
        ...
        for group_config in self._grouped_embedding_configs:
            for embedding_table in group_config.embedding_tables:
                if (
                    embedding_table.total_num_buckets is not None
                    and embedding_table.use_virtual_table
                ):
        ...
        for i, hash_size in enumerate(feature_hash_sizes):
            ...
            if feature_total_num_buckets is not None:
                if feature_total_num_buckets[i] % self._world_size != 0:
                    ...
                    ), "has_uneven_virtual_tables should be True if uneven buckets per rank is present"
...
class RwSparseFeaturesWriteDist(BaseSparseFeaturesWriteDist[KeyedJaggedTensor]):
    ...
    def __init__(
        ...
        is_sequence: bool = False,
        keep_original_indices: bool = False,
        has_uneven_buckets_per_rank: bool = False,
        zch_sizes: Optional[List[int]] = None,
    ) -> None:
        super().__init__()
        self._world_size: int = pg.size()
        ...
        for i, hash_size in enumerate(feature_hash_sizes):
            ...
            if feature_total_num_buckets is not None:
                if feature_total_num_buckets[i] % self._world_size != 0:
                    assert (
                        has_uneven_buckets_per_rank
                    ), "has_uneven_buckets_per_rank should be True if uneven buckets per rank is present"
            ...
            if has_uneven_buckets_per_rank:
                row_pos_num_buckets.append(block_divisor)
                if hash_size == 0:
                    assert (
                        zch_sizes is not None
