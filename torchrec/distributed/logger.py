...
Key Components:
    ...
    - _get_input_from_func: Helper to extract function arguments for logging.
    - _get_msg_dict: Helper to create log message dictionaries with distributed
      context.
...
global _no_fileter_logger, single_rank_logger, all_rank_logger, capped_logger, method_logger
_no_fileter_logger = _get_or_create_logger(_NoFilterLogger)
    ...
    def decorator(func: Callable[_P, _T]) -> Callable[_P, _T]:  # pyre-ignore
        ...
        def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _T:
            ...
            try:
                ...
                _no_fileter_logger.fatal(
                    f"Torchrec logger: Failed in static logger: {error}"
                )
