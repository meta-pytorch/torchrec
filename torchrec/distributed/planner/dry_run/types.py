...
class DryRunRequest(ShardingPlanRequest):
    ...
    def fingerprint(self, sku: str) -> str:
        ...
        pod_size, hbm_gb, ddr_gb, training_framework, launcher_hardware, and
        constraints -- so this method only adds the SKU-specific delta. Building
        on request_hash (rather than re-listing the base params) keeps the two in
        sync and ensures constraints/launcher_hardware participate in the hash;
        omitting them previously let configs that produce different plans collide.
        ...
        if self.per_sku_overrides and sku in self.per_sku_overrides:
            o = self.per_sku_overrides[sku]
            override = {
                "hbm_gb": o.hbm_gb,
                "ddr_gb": o.ddr_gb,
                "intra_host_bw": o.intra_host_bw,
                "inter_host_bw": o.inter_host_bw,
            }
        parts = {
