...
class SKUAwareStorageReservation(StorageReservation):
    ...
    dominant cause of planner OOM/failures under hardware fungibility (TC_ANY).
    ...
    def reserve(
        ...
        # "could not place tables" failure. Check the smallest device so a
        # heterogeneous topology cannot bypass the guard.
        min_hbm_device = min(reserved_topology.devices, key=lambda d: d.storage.hbm)
        available_hbm = min_hbm_device.storage.hbm
        if self._reserved_bytes >= available_hbm:
            ...
            insufficient_storage_solution = (
                ...
                "exceeds the available hbm storage per rank "
                f"({storage_repr_in_gb(min_hbm_device.storage)}), leaving no hbm "
