...
class DefaultPlannerExecutor(PlannerExecutor):
    ...
    def run(
        ...
        if is_planning_rank:
            ...
            logger.info(
                "[planner] %s planned in %.0fms: %d table(s), %d shard(s), "
                "peak/rank hbm=%.1fGiB ddr=%.1fGiB",
                sku,
                solve_time_ms,
                # best_plan holds one ShardingOption per table, so its length is a
                # table count; the shard count is the sum over each option's shards.
                # The original reported the former under the latter's label, which
                # understates a column-wise or row-wise plan by a large factor.
                len(best_plan),
                sum(len(option.shards) for option in best_plan),
