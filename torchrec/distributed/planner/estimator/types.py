...
class HardwarePerfConfig:
    ...
    def coefficients(self) -> PerfCoefficientConfig:
        ...
        sharding_types = [
            (ShardingType.TABLE_WISE.value, "table_wise"),
            (ShardingType.ROW_WISE.value, "row_wise"),
            (ShardingType.TABLE_ROW_WISE.value, "table_row_wise"),
            (ShardingType.COLUMN_WISE.value, "column_wise"),
            (ShardingType.DATA_PARALLEL.value, "data_parallel"),
        ]

        config_kwargs: Dict[str, Optional[EstimatorPerfCoefficients]] = {}

        for sharding_type_value, attr_name in sharding_types:
            fwd_coeff = get_fwd_coefficient(self, sharding_type_value)
            bwd_coeff = get_bwd_coefficient(self, sharding_type_value)

            if fwd_coeff is not None or bwd_coeff is not None:
                config_kwargs[attr_name] = EstimatorPerfCoefficients(
        ...
        self._coefficients = PerfCoefficientConfig(
            table_wise=config_kwargs.get("table_wise"),
            row_wise=config_kwargs.get("row_wise"),
            table_row_wise=config_kwargs.get("table_row_wise"),
            column_wise=config_kwargs.get("column_wise"),
            data_parallel=config_kwargs.get("data_parallel"),
            prefetch=prefetch,
        )
...
class ShardPerfContext:
    ...
    def build_shard_perf_contexts(
        ...
        batch_sizes = (
            list(constraints[sharding_option.name].batch_sizes)
            if constraints
