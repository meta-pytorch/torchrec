...
class EmbeddingEnumerator(Enumerator):
    ...
    def _filter_compute_kernels(
        ...
        if EmbeddingComputeKernel.TRITON_UVM.value in filtered_compute_kernels:
            filtered_compute_kernels = [
                k
                for k in filtered_compute_kernels
                if k != EmbeddingComputeKernel.FUSED_UVM.value
            ]
...
def _extract_constraints_for_param(
    ...
    if constraints and constraints.get(name):
        ...
        cache_params = copy.deepcopy(constraints[name].cache_params)
