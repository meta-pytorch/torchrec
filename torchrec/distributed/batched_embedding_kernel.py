...
class TritonBatchedFusedEmbeddingBag(
    ...
    @property
    def _param_per_table(self) -> Dict[str, TableBatchedEmbeddingSlice]:
        # For Triton TBE, we return the split embedding weights as TableBatchedEmbeddingSlice
        ...
        for config, weight in zip(
            ...
            if config.name not in result:
                result[config.name] = TableBatchedEmbeddingSlice(
                    data=weight.view(-1),
                    start_offset=0,
                    end_offset=weight.numel(),
                    num_embeddings=config.local_rows,
                    embedding_dim=config.local_cols,
                )
        return result
