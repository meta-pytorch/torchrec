...
_MODULE_CASES: Tuple[_GoldenCase, ...] = (
    ...
    # The ITEP rows above already record these key paths, so a rename inside
    # either collection fails there too. These rows keep the coverage if the
    # ITEP wrappers are ever removed.
    _GoldenCase(
...
_METRIC_SPECS: Tuple[_GoldenCase, ...] = (
    ...
    _GoldenCase.for_metric(SessionPairwiseAUCMetric),
