...
"""
This test suite guards against breaking changes to metric state_dict keys/FQNs
that would cause checkpoint loading failures. It compares current metric FQNs
against a golden baseline and fails if:
1. Any persistent buffer FQN is removed (BREAKING - old checkpoints can't load)
2. Any state_dict key is removed (BREAKING - old checkpoints can't load)
3. Any state_dict key is added WITHOUT proper load_state_dict hooks

For added keys to be acceptable, the metric must implement load_state_dict hooks that:
- Handle missing keys gracefully (backward compatibility with old checkpoints)
...
class MetricFQNBackwardCompatibilityTest(unittest.TestCase):
    ...
    def _check_metric_compatibility(
        ...
        **kwargs: Any,
        ...
        """
        Check a single metric for backward compatibility.

        Raises assertion error if incompatible changes are detected.

        Compatibility requirements:
        1. Removed keys/FQNs always fail (breaks loading old checkpoints)
        2. Added keys/FQNs fail UNLESS the metric has proper load_state_dict hooks
           that handle missing keys (backward compat) and ignore extra keys (forward compat)
        """
        key = get_metric_snapshot_key(metric_class, compute_mode, variant)
        ...
        if added_keys or added_fqns:
            ...
            if not has_backward_compat:
                self.fail(
                    ...
                    "New keys will cause OLD code to fail loading NEW checkpoints (forward incompatible). "
                    "To fix: implement load_state_dict hooks to handle extra keys gracefully, "
                    "or update the golden snapshot if this is intentional."
                )
    ...
    def _verify_backward_compatibility(
        ...
        **kwargs: Any,
        ...
        """
        Verify that the metric can load OLD checkpoints (missing new keys).

        Returns True if the metric has hooks that handle missing keys,
        allowing strict=True to pass even when checkpoint is missing new keys.
        """
        if not added_keys:
            return True

        use_tensor_task = kwargs.pop("use_tensor_task", False)
        tasks = [create_test_task("test_task", with_tensor_name=use_tensor_task)]
