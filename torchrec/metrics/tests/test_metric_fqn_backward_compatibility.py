...
class _GoldenCase:
    ...
    # The golden file's key. The hand-written ids are snake_case, so a class
    # rename leaves them alone. for_metric derives the rest from the class
    # name: those survive a file move but not a rename, which the
    # regeneration test reports as an orphaned entry.
    stable_id: str
...
_ITEP_TABLE_NAME = "table1"
_ITEP_FEATURE_NAME = "feature1"


def _make_itep_module() -> GenericITEPModule:
    """The only ITEP configuration that builds without a GPU.

    With no lookups it registers no buffers, so the wrappers below contribute
    just their own `_iter`. The pruning buffers need a CUDA-only fbgemm op.
    """
    return GenericITEPModule(
        table_name_to_unpruned_hash_sizes={_ITEP_TABLE_NAME: 100},
        lookups=None,
    )
...
_MODULE_CASES: Tuple[_GoldenCase, ...] = (
    ...
    # These two wrap a collection, so their entries carry its weight names as
    # well as ITEP's own `_iter`. A rename inside the collection fails these
    # rows, which is a real checkpoint break reported under an ITEP id.
    _GoldenCase(
        ...
        lambda: ITEPEmbeddingBagCollection(
            embedding_bag_collection=EmbeddingBagCollection(
                tables=[
                    EmbeddingBagConfig(
                        name=_ITEP_TABLE_NAME,
                        embedding_dim=4,
                        num_embeddings=50,
                        feature_names=[_ITEP_FEATURE_NAME],
                    )
                ],
                device=torch.device("cpu"),
            ),
...
class MetricCoverageTest(unittest.TestCase):
    ...
    def test_all_rec_metric_modules_are_covered(self) -> None:
        ...
        discovered_modules: Set[str] = {
            cls.__name__ for cls in _discover_subclasses(RecMetricModule)
        }
        ...
        if missing_modules:
            self.fail(
                ...
                f"backward compatibility tests: {sorted(missing_modules)}.\n\n"
                f"To fix this:\n"
