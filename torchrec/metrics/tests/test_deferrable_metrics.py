...
class TestDeferrableMetrics(unittest.TestCase):
    ...
    def test_resolve_returns_defensive_copy(self) -> None:
        dm = DeferrableMetrics({"a": 1})
        d = dm.resolve()
        d["a"] = 999
        self.assertEqual(dm.resolve()["a"], 1)

    def test_subscribe_delivers_defensive_copy(self) -> None:
        dm = DeferrableMetrics({"a": 1})
        received: list[dict] = []
        dm.subscribe(lambda d: received.append(d))
        received[0]["a"] = 999
    ...
    def test_multiple_subscribers(self) -> None:
        ...
        dm.subscribe(lambda d: received_1.append(d))
        dm.subscribe(lambda d: received_2.append(d))
        f.set_result({"val": 42})
        self.assertEqual(received_1, [{"val": 42}])
        self.assertEqual(received_2, [{"val": 42}])

    def test_subscribe_before_update(self) -> None:
        f: Future[dict] = Future()
        dm = DeferrableMetrics(f)
        received: list[dict] = []
        dm.subscribe(lambda d: received.append(d))
        dm.update({"extra": 99})
        f.set_result({"a": 1})
        self.assertEqual(received, [{"a": 1, "extra": 99}])
        self.assertTrue(dm.is_resolved())
    ...
    def test_chained_updates(self) -> None:
        ...
        dm.subscribe(lambda d: received.append(d))
        f.set_result({"a": 1})
        self.assertEqual(received, [{"a": 1, "b": 2, "c": 3, "d": 4}])

    def test_subscribe_callback_exception(self) -> None:
        f: Future[dict] = Future()
        dm = DeferrableMetrics(f)
        received: list[dict] = []
        errors: list[Exception] = []

        def failing_cb(d: dict) -> None:  # pyre-ignore[2]
            raise RuntimeError("cb failed")

        dm.subscribe(failing_cb, on_error=lambda e: errors.append(e))
        dm.subscribe(lambda d: received.append(d))
        f.set_result({"a": 1})
        self.assertEqual(len(errors), 1)
    ...
    def test_update_invalid_type(self) -> None:
        dm = DeferrableMetrics({"a": 1})
        with self.assertRaisesRegex(TypeError, "got list"):
            dm.update([1, 2, 3])  # pyre-ignore[6]
