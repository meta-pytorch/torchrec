    ...
    **Precision Formula:**
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._label_names: List[str] = label_names if label_names is not None else []
        self._num_labels: Optional[int] = num_labels  # Store num_labels from config
        self._per_label_states_initialized = False

        # Generate default label names if empty list
    ...
    def _decode_integer_to_labels(
        ...
        Args:
            tensor: Integer-encoded tensor of shape [batch_size] or [batch_size, 1]
            num_labels: Number of labels (bits) to decode from each integer

        Returns:
            Binary tensor of shape [batch_size, num_labels] with values 0 or 1

        Example:
            >>> _decode_integer_to_labels(torch.tensor([5, 3]), num_labels=3)
            tensor([[1, 0, 1],   # 5 = 101 in binary (LSB first)
                    [1, 1, 0]])  # 3 = 011 in binary (LSB first)
        """
    ...
    def _prepare_inputs(
        ...
        Args:
            predictions: Integer-encoded predictions of shape [batch_size] or [batch_size, 1]
            labels: Integer-encoded labels of shape [batch_size] or [batch_size, 1]
            weights: Optional weight tensor of shape [batch_size]

        Returns:
            Tuple of (predictions, labels, weights) where predictions and labels
            are decoded to shape [batch_size, num_labels] and weights are expanded
            to match.
        """
    ...
    def _update_label_states(
        ...
        Args:
            label_idx: Index of the label in the prediction tensor
            label_name: Name of the label for state lookup
            predictions: Prediction tensor of shape (batch_size, num_labels)
            labels: Label tensor of shape (batch_size, num_labels)
            weights: Weight tensor of shape (batch_size, num_labels)
        """
    ...
    def update(
        ...
        *,
        ...
        Args:
            predictions: Integer-encoded predictions of shape [batch_size] or [batch_size, 1].
                Each integer represents a binary vector of label predictions.
            labels: Integer-encoded labels of shape [batch_size] or [batch_size, 1].
                Each integer represents a binary vector of ground truth labels.
            weights: Optional weight tensor of shape [batch_size]
        """
        if predictions is None:
            raise RecMetricException("Predictions required")

        if self._num_labels != len(self._label_names):
            raise RecMetricException(
                f"Number of labels ({len(self._label_names)}) does not match num_labels ({self._num_labels})"
            )
