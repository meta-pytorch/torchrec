...
"""
TorchX component for running TorchRec DLRM training on Azure AKS.

This module provides a TorchX AppDef for distributed DLRM training
on Azure using AKS (Azure Kubernetes Service) with GPU VMs.

Example usage:
    torchx run -s kubernetes -cfg namespace=default \\
        cloud_deployment.azure.torchx_azure_component:run_dlrm_azure \\
        --num_trainers 16 \\
...
def run_dlrm_azure(
    num_trainers: int = 8,
    dataset_path: str = "/mnt/netapp/criteo/",
    batch_size: int = 2048,
    ...
    *script_args: str,
    ...
    """
    TorchX AppDef for running DLRM training on Azure AKS.

    This component configures distributed training optimized for Azure ND96asr_v4
    VMs with 8x A100 GPUs each. It sets up appropriate NCCL environment
    variables for InfiniBand networking.

    Args:
        num_trainers: Total number of GPU trainers. Must be multiple of 8 for
            multi-node training. Default: 8 (single node).
        dataset_path: Path to preprocessed Criteo dataset on Azure NetApp Files.
            Default: /mnt/netapp/criteo/
        ...
        checkpoint_path: Optional Azure Blob path for saving checkpoints.
    ...
    Example:
        >>> app = run_dlrm_azure(num_trainers=16, batch_size=4096)
        >>> # Submit to Azure AKS
    ...
    # Azure ML Container image with PyTorch and TorchRec
    ...
    # Environment variables optimized for Azure InfiniBand
    env = {
    ...
    return ddp(
        *train_args,
        name="torchrec-dlrm-azure",
        image=image,
        # Azure ND96asr_v4 VM specs
        cpu=96,
        gpu=8,
        memMB=900000,  # ~900 GB
        script=entrypoint,
...
def run_dlrm_azure_spot(
    num_trainers: int = 8,
    dataset_path: str = "/mnt/netapp/criteo/",
    checkpoint_interval: int = 1000,
    checkpoint_path: str = "https://storage.blob.core.windows.net/checkpoints/",
) -> specs.AppDef:
    """
    TorchX AppDef for DLRM training on Azure Spot VMs.

    This variant is optimized for cost savings using Spot VMs with
    frequent checkpointing for fault tolerance.

    Args:
        num_trainers: Total number of GPU trainers.
        dataset_path: Path to dataset on Azure NetApp Files.
        checkpoint_interval: Save checkpoint every N iterations.
        checkpoint_path: Azure Blob URL for checkpoint storage.

    Returns:
        specs.AppDef: TorchX application definition.
    """
    return run_dlrm_azure(
        num_trainers,
        dataset_path,
        2048,  # batch_size
        1.0,  # learning_rate
